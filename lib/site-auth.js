 ...
 * student information.
 *
 * Three passwords, three roles:
 *   SITE_PASSWORD     → role "parent"  — can view everything
 *   CARLINE_PASSWORD  → role "carline" — can view everything AND use /carline.
 *                       Handed to PreK teachers. Deliberately CANNOT reach
 *                       /admin, so a code shared with staff never exposes the
 *                       admin page, the data push, or the document uploader.
 *   ADMIN_PASSWORD    → role "admin"   — everything, including /admin
 *
...
const CARLINE_PASSWORD = process.env.CARLINE_PASSWORD || 'CarlinePreK2026';
...
const SECRET = process.env.SESSION_SECRET ||
    crypto.createHash('sha256')
        .update(`${SITE_PASSWORD}::${ADMIN_PASSWORD}::${CARLINE_PASSWORD}`).digest('hex');
...
/** Returns 'admin' | 'carline' | 'parent' | null */
function roleFromRequest(req) {
    ...
    if (role === 'carline') return 'carline';
    return 'parent';
}
...
function roleFromPassword(supplied) {
    ...
    if (safeEqual(supplied, CARLINE_PASSWORD)) return 'carline';
...
module.exports = {
    ...
    usingDefaultCarlinePassword: () => !process.env.CARLINE_PASSWORD
};
